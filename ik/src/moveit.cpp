        ...
        // [aubo state]
        {
            ...
            if (ret == aubo_robot_namespace::InterfaceCallSuccCode) {
                for (size_t i = 0; i < 6; ++i) {
                    this->last_sent_aubo_point[i] = joint_status[i].jointPosJ;
                }
            } else {
...
private:
    void callback_moveit_plan() {
        ...
        const auto to = [&] {
            ...
            msg.position.x = pos_estimated_effector[0];
            msg.position.y = pos_estimated_effector[1];
            msg.position.z = pos_estimated_effector[2];
